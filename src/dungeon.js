...
export default class Dungeon {
  ...
  generate() {
    ...
    for (let i = 0; i < this.rooms.length; i++) {
      ...
      for (let j = 0; j < targets.length; j++) {
        ...
        if (random < 1) {
          ...
          if (rand === 1) {
            this.addDoor(this.findNewDoorLocation(room, result.target), rand);
          } else {
            this.addDoor(this.findNewDoorLocation(room, result.target), rand);
          }
    ...
    for (let it = 0; it < this.items; it++) {
      this.addItem(tiles.item)
    }

    for (let e = 0; e < this.enemies; e++) {
      this.addEnemies(tiles.enemy)
    }
    if (this.createAltar == true) {
      this.addAltar(tiles.altar)
    }
  }
    ...
    while (iter-- > 0) {
      ...
      if (this.addRoom(room)) {
        ...
        if (rand === 1) {
          ...
          for (let xDoor = 0; xDoor <= room.size.x; xDoor++){
            this.addDoor(this.findNewDoorLocation(room, result.target), rand);
          }
          for (let yDoor = 0; yDoor <= room.size.y; yDoor++){
            this.addDoor(this.findNewDoorLocation(room, result.target), rand);
          }
        }
        break;
  ...
  addStairs(type) {
    ...
    for (let y = 1; y < room.size.y - 2; y++) {
      for (let x = 1; x < room.size.x - 2; x++) {
        ...
        if (
          ...
          room.tiles[y][x + 1] === tiles.door ||
          room.tiles[y - 1][x] === tiles.doorClosed ||
          room.tiles[y + 1][x] === tiles.doorClosed ||
          room.tiles[y][x - 1] === tiles.doorClosed ||
          room.tiles[y][x + 1] === tiles.doorClosed
        ) {
          continue;
        }
    ...
    if (candidates.length > 0) {
      ...
      alert("oops there was an error with the room generation. Please send me a shout to let me know!")
      //this.addStairs(tiles.stairsDown);
    }
  }
  ...
  addAltar(type) {
    ...
    if (candidates.length > 0) {
      ...
      this.addAltar(tiles.item)
    }
  }


  addEnemies(type, index) {
    let room = null;
    ...
    for (let y = 1; y < room.size.y - 2; y++) {
      for (let x = 1; x < room.size.x - 2; x++) {
        ...
        // add it to the candidate list
        candidates.push({ x: x, y: y, pos: room.pos });
      }
    }
    ...
    if (candidates.length > 0) {
      let loc = randomElement(candidates);
      room.tiles[loc.y][loc.x] = type;
      this.entities.enemies.push({ y: loc.y + loc.pos.y, x: loc.x + loc.pos.x, hasBeenSeen: false, active: false, enemyData: {}, pos: { x: 0, y: 0 }, id: 0 })
    } else {
      return;
    }
  }
